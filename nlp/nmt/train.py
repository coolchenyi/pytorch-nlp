...
nlp.nmt.opts.model_opts(parser)
nlp.nmt.opts.train_opts(parser)
...
def report_func(epoch, batch, num_batches,
    ...
    if batch % opt.report_every == -1 % opt.report_every:
        ...
        report_stats = nlp.nmt.onmt.Statistics()
...
def make_train_data_iter(train_dataset, opt):
    ...
    """
    return nlp.nmt.onmt.io.OrderedIterator(
...
def make_loss_compute(model, tgt_vocab, dataset, opt):
    ...
    """
    if opt.copy_attn:
        compute = nlp.nmt.onmt.modules.CopyGeneratorLossCompute(
            model.generator, tgt_vocab, dataset, opt.copy_attn_force)
    else:
        compute = nlp.nmt.onmt.Loss.NMTLossCompute(model.generator, tgt_vocab,
                                           opt.label_smoothing)
...
def train_model(model, train_dataset, valid_dataset,
    ...
    trainer = nlp.nmt.onmt.Trainer(model, train_iter, valid_iter,
...
def load_dataset(data_type):
    ...
    if pts:
        ...
        dataset = nlp.nmt.onmt.io.ONMTDatasetBase.coalesce_datasets(datasets)
    else:
...
def collect_report_features(fields):
    src_features = nlp.nmt.onmt.io.collect_features(fields, side='src')
    tgt_features = nlp.nmt.onmt.io.collect_features(fields, side='tgt')
...
def build_model(model_opt, opt, fields, checkpoint):
    ...
    model = nlp.nmt.onmt.ModelConstructor.make_base_model(model_opt, fields,

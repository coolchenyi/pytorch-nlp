...
from nlp.chatbot.load import load_prepare_data
from nlp.chatbot.load import SOS_token, EOS_token, PAD_token
from nlp.chatbot.model import EncoderRNN, AttnDecoderRNN
from nlp.chatbot.config import MAX_LENGTH, USE_CUDA, teacher_forcing_ratio
...
def batch_2_train_data(voc, pair_batch, reverse):
    if reverse:
...
def mask_nll_loss(input, target, mask):
    n_total = mask.sum()
...
def train(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding,
    ...
    if use_teacher_forcing:
        ...
        for t in range(max_target_len):
            ...
            topv, topi = decoder_output.data.topk(1)  # [batch_size, 1]
...
def train_iters(corpus, reverse, n_iteration, learning_rate, batch_size, n_layers, hidden_size,
               print_every, save_every, save_dir, load_file_name=None, decoder_learning_ratio=5.0):
    voc, pairs = load_prepare_data(corpus, save_dir)
    ...
    try:
        ...
        training_batches = [batch_2_train_data(voc, [random.choice(pairs) for _ in range(batch_size)], reverse)
    ...
    for iteration in tqdm(range(start_iteration, n_iteration + 1)):
        ...
        if iteration % print_every == 0:
            ...
            print('iteration:%d, process:%d, avg_loss:%.4f' % (iteration, iteration / n_iteration * 100, print_loss_avg))
            print_loss = 0

...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_seq, last_hidden, encoder_outputs):
        ...
        embedded = self.embedding(input_seq)
        embedded = self.embedding_dropout(embedded)   # [1, batch_size, 512]
        ...
        # encoder_outputs [MAX_LENGTH, batch_size, 512]
        context = attn_weights.bmm(encoder_outputs.transpose(0, 1))  # [batch_size, 1, 512]

        # Attentional vector using the RNN hidden state and context vector
        # concatenated together (Luong eq. 5)
        rnn_output = rnn_output.squeeze(0)    # [batch_size, 512]
        context = context.squeeze(1)   # [batch_size, 512]
        concat_input = torch.cat((rnn_output, context), 1)   # [batch_size, 1024]
        concat_output = F.tanh(self.concat(concat_input))    # [batch_size, 512]

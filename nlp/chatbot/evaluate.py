...
import random
from nlp.chatbot.train import indexes_from_sentence
from nlp.chatbot.load import SOS_token, EOS_token
from nlp.chatbot.load import MAX_LENGTH, load_prepare_data, Voc
from nlp.chatbot.model import *
from nlp.chatbot.config import USE_CUDA
...
class Sentence:
    ...
    def add_topk(self, topi, topv, decoder_hidden, beam_size, voc):
        topv = torch.log(topv)
...
def evaluate(encoder, decoder, voc, sentence, beam_size, max_length=MAX_LENGTH):
    indexes_batch = [indexes_from_sentence(voc, sentence)]  # [1, seq_len]
...
def run_test(n_layers, hidden_size, reverse, model_file, beam_size, input, corpus, save_dir):
